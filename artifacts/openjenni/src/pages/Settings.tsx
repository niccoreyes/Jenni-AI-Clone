  ...
  const handleSave = () => {
    updateSettings.mutate({
      data: {
        ...
        model: form.model,
        defaultCitationStyle: form.defaultCitationStyle as "APA7" | "MLA9" | "Chicago17" | "IEEE" | "Harvard",
        language: form.language,
        userRole: form.userRole as "undergraduate" | "graduate" | "phd" | "researcher" | "professional",
  ...
  return (
    <div className="min-h-screen bg-background">
      <header className="border-b border-border bg-card px-6 py-4 flex items-center justify-between">
        ...
        <Button size="sm" onClick={handleSave} disabled={updateSettings.isPending} data-testid="button-save-settings">
      ...
      <main className="max-w-2xl mx-auto px-6 py-8 space-y-8">
        <section>
          <h2 className="text-base font-semibold text-foreground mb-1">AI Provider (BYOK)</h2>
          <p className="text-sm text-muted-foreground mb-4">Connect your own API key to use AI features.</p>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              <Label>Model</Label>
              <Select value={form.model} onValueChange={m => setForm(f => ({ ...f, model: m }))}>
                <SelectTrigger data-testid="select-model">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {(PROVIDER_MODELS[form.provider] ?? ["custom-model"]).map(m => (
        ...
        <section>
          <h2 className="text-base font-semibold text-foreground mb-4">Academic Preferences</h2>
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <Label>Default Citation Style</Label>
              <Select
                value={form.defaultCitationStyle}
                onValueChange={v => setForm(f => ({ ...f, defaultCitationStyle: v }))}
              >
        ...
        <section>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-foreground">AI Autocomplete</p>
                <p className="text-xs text-muted-foreground">Press Ctrl+J to get AI writing suggestions</p>
              </div>
              <Switch
                checked={form.autocompleteEnabled}
                onCheckedChange={v => setForm(f => ({ ...f, autocompleteEnabled: v }))}
                data-testid="toggle-autocomplete"
              />
            </div>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-foreground">Citation Manager</p>
                <p className="text-xs text-muted-foreground">Manage and format academic citations</p>
              </div>
              <Switch
                checked={form.citationsEnabled}
                onCheckedChange={v => setForm(f => ({ ...f, citationsEnabled: v }))}
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Upload and analyze research papers</p>
              </div>
              <Switch
                checked={form.pdfChatEnabled}
                onCheckedChange={v => setForm(f => ({ ...f, pdfChatEnabled: v }))}
