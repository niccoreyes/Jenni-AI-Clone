...
export default function Editor() {
  ...
  const [citationStyle, setCitationStyle] = useState<"APA7" | "MLA9" | "Chicago17" | "IEEE" | "Harvard">("APA7");
  ...
  const [citationForm, setCitationForm] = useState({ author: "", year: "", title: "", journal: "", doi: "" });
  ...
  const { data: citations } = useListCitations({ documentId: docId }, {
    query: { enabled: !!docId, queryKey: getListCitationsQueryKey({ documentId: docId }) }
  });
  ...
  const createCitation = useCreateCitation({
    mutation: {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: getListCitationsQueryKey({ documentId: docId }) });
        queryClient.invalidateQueries({ queryKey: getGetDocumentStatsQueryKey() });
        setAddCitationOpen(false);
        setCitationForm({ author: "", year: "", title: "", journal: "", doi: "" });
  ...
  const scheduleSave = useCallback((newContent: string, newTitle: string, newStyle: string) => {
    if (saveTimerRef.current) clearTimeout(saveTimerRef.current);
    saveTimerRef.current = setTimeout(() => {
      updateDocument.mutate({
        id: docId,
        data: { content: newContent, title: newTitle, citationStyle: newStyle as "APA7" | "MLA9" | "Chicago17" | "IEEE" | "Harvard" }
      });
    }, 2000);
  ...
  const handleParaphrase = (mode: string) => {
    ...
    paraphrase.mutate(
      { data: { text: selectedText, mode: mode as "simplify" | "academic" | "expand" | "shorten" | "active" | "passive", documentId: docId } },
      {
        onSuccess: (result) => {
  ...
  const applyParaphrase = () => {
    const newContent = content.slice(0, selectionStart) + paraphraseResult + content.slice(selectionEnd);
    setContent(newContent);
  ...
  const sendChatMessage = () => {
    ...
    aiChat.mutate(
      ...
      {
        onSuccess: (result) => {
          setChatMessages(h => [...h, { role: "assistant", content: result.response }]);
        },
        onError: () => {
          setChatMessages(h => [...h, { role: "assistant", content: "Failed to get response. Please check your API key in Settings." }]);
        }
      }
    );
  };
    ...
    generateOutline.mutate(
      {
        data: {
          ...
          documentType: outlineDocType as "research_paper" | "essay" | "literature_review" | "thesis" | "report",
        }
      },
      {
        onSuccess: (result) => {
  ...
  const renderOutline = (sections: OutlineSection[], depth = 0): React.ReactNode => (
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          ...
          <Button onClick={() => setLocation("/")} size="sm">Back to Dashboard</Button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="h-screen bg-background flex flex-col overflow-hidden">
      <header className="border-b border-border bg-card px-4 py-2.5 flex items-center gap-3 flex-shrink-0">
        <Button variant="ghost" size="sm" className="h-7 px-2 text-xs" onClick={() => setLocation("/")} data-testid="button-back">
          Back
        </Button>
        ...
        <div className="flex items-center gap-2 ml-auto">
          <Select value={citationStyle} onValueChange={handleStyleChange}>
            <SelectTrigger className="h-7 text-xs w-32" data-testid="select-editor-citation-style">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="flex-1 flex flex-col overflow-hidden">
          {selectedText && (
            <div className="border-b border-border bg-muted/50 px-4 py-2 flex items-center gap-1 flex-shrink-0">
              <span className="text-xs text-muted-foreground mr-2">Rewrite:</span>
              {["simplify", "academic", "expand", "shorten", "active"].map(mode => (
                <Button
                  key={mode}
                  variant="outline"
                  size="sm"
            ...
            {doc.targetWordCount && (
              <>
                ...
                <div className="flex-1 max-w-32">
                  <Progress value={Math.min(100, (wordCount / doc.targetWordCount) * 100)} className="h-1" />
                </div>
              </>
            )}
        ...
        <div className="w-72 border-l border-border bg-card flex flex-col overflow-hidden flex-shrink-0">
          <div className="border-b border-border flex-shrink-0">
            <div className="flex">
              {(["chat", "citations", "pdfs", "outline"] as SidebarTab[]).map(tab => (
                <button
                  key={tab}
                  ...
                  data-testid={`tab-${tab}`}
                >
                  {tab === "pdfs" ? "PDFs" : tab === "chat" ? "AI Chat" : tab.charAt(0).toUpperCase() + tab.slice(1)}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {activeTab === "chat" && (
              <div className="flex flex-col h-full">
                <div className="flex-1 p-3 space-y-3 overflow-y-auto">
                  ...
                  {aiChat.isPending && (
                    <div className="bg-muted rounded-lg p-2.5 mr-4">
            ...
            {activeTab === "citations" && (
              <div className="p-3 space-y-3">
                ...
                {citations?.map(cit => (
                  <div key={cit.id} className="border border-border rounded-lg p-2.5 group" data-testid={`citation-${cit.id}`}>
                    <p className="text-xs font-medium text-foreground">{cit.author}</p>
                    <p className="text-xs text-muted-foreground mt-0.5 line-clamp-2">{cit.title}</p>
                    {cit.year && <p className="text-xs text-muted-foreground">{cit.year}</p>}
                    <div className="flex items-center justify-between mt-1.5">
                      <span className="text-xs text-primary">{cit.citationStyle}</span>
                      <Button
                        variant="ghost"
                        size="sm"
                        className="h-5 px-1 text-xs text-destructive opacity-0 group-hover:opacity-100"
                        onClick={() => deleteCitation.mutate({ id: cit.id })}
            ...
            {activeTab === "pdfs" && (
              <div className="p-3 space-y-3">
                ...
                {pdfs?.length === 0 && (
                  <p className="text-xs text-muted-foreground text-center py-4">No PDFs uploaded yet.</p>
                )}
                {pdfs?.map(pdf => (
                  <div key={pdf.id} className="border border-border rounded-lg p-2.5 group" data-testid={`pdf-${pdf.id}`}>
                    <p className="text-xs font-medium text-foreground truncate">{pdf.originalName}</p>
            ...
            {activeTab === "outline" && (
              <div className="p-3 space-y-3">
                <div className="space-y-2">
                  ...
                  />
                  <Select value={outlineDocType} onValueChange={setOutlineDocType}>
                    <SelectTrigger className="h-8 text-xs" data-testid="select-outline-doc-type">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                  ...
                  <Button
                    ...
                    disabled={generateOutline.isPending || !outlineTopic || !outlineThesis}
        ...
        <DialogContent>
          ...
          <div className="space-y-3 py-2">
            <div>
              ...
              <p className="text-sm text-muted-foreground bg-muted rounded p-2">{selectedText}</p>
            </div>
            <div>
              <p className="text-xs font-medium text-muted-foreground mb-1">Rewritten</p>
              <Textarea
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setParaphraseOpen(false)}>Cancel</Button>
            <Button onClick={applyParaphrase} data-testid="button-apply-paraphrase">Apply</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={addCitationOpen} onOpenChange={setAddCitationOpen}>
        <DialogContent>
          ...
          <div className="space-y-3 py-2">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <Input
                  ...
                  onChange={e => setCitationForm(f => ({ ...f, author: e.target.value }))}
                  placeholder="Smith, J."
              ...
              <div className="space-y-1">
                ...
                <Input
                  ...
                  onChange={e => setCitationForm(f => ({ ...f, year: e.target.value }))}
                  placeholder="2024"
                  className="text-sm h-8"
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                onChange={e => setCitationForm(f => ({ ...f, title: e.target.value }))}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <Input
                  ...
                  onChange={e => setCitationForm(f => ({ ...f, journal: e.target.value }))}
              ...
              <div className="space-y-1">
                ...
                <Input
                  ...
                  onChange={e => setCitationForm(f => ({ ...f, doi: e.target.value }))}
          ...
          <DialogFooter>
            ...
            <Button
              onClick={addCitation}
              disabled={!citationForm.author || !citationForm.title || createCitation.isPending}
